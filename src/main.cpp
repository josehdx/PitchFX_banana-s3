...
void MidiTask(void * pvParameters) {
    ...
    for(;;) {
        ...
        // 1. Calculate how many semitones the Whammy is bending based on the pedal position
        float currentWhammyShift = maxSemitones * normalizedThrow;
        
        // 2. Determine if the Capo base shift is active 
        // (Make sure you declared 'float capoSemitones = -2.0f;' at the top of your file!)
        float currentCapoShift = isCapoMode ? capoSemitones : 0.0f;
        
        // 3. Add them together to get the total semitone shift
        float totalSemitones = currentCapoShift + currentWhammyShift;
        
        // 4. Convert the total shift into octaves and calculate the final DSP multiplier!
        pitchShiftFactor = powf(2.0f, totalSemitones / 12.0f);
