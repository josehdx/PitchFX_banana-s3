...
// --- CORE 1: PHASE-LOCKED DSP TASK ---
    ...
    i2s_event_t event;
        ...
        // 1. Put Core 1 to sleep until the DMA hardware sends an interrupt to the queue
        if (xQueueReceive(i2s_event_queue, &event, portMAX_DELAY) == pdPASS) {
            
            // 2. Only run the heavy DSP math if the interrupt says "Audio Input Buffer is Full!"
            if (event.type == I2S_EVENT_RX_DONE) {

                // ---  SNAPSHOT VOLATILES TO CPU REGISTERS ---
            ...
            const float _pitchFactor = pitchShiftFactor;
                ...
                // Because the DMA just woke us up, this read happens instantly without blocking
      ...
      float liveMag = fast_mag(re, im);
            ...
            float binFreq = (float)lastPeak * FREQ_CONST; // 1-cycle multiply

            // Branch prediction hint speeds up live playing
            if (unlikely(_frozen)) {
                // --- FREEZE MODE ---
            ...
            int targetBin = (int)((i * mainFactor) + 0.5f); // Fast float-to-int cast
            
            if (targetBin < SAMPLES / 2) {
                newMag[targetBin] += _frozen ? frozenMag[i] : liveMag;
                float shiftedFreq = (binFreq + (deltaPhase * INV_HOP)) * mainFactor; // Fast multiply
            ...
            if (_feedback > 0.0f) {
                ...
                int fbTargetBin = (int)((i * fbFactor) + 0.5f); // Fast float-to-int cast
          ...
          // --- SYNTHESIS PATH 3: Polyphonic Harmonizer ---
            if (_harmonizer && fabsf(_pitchFactor - 1.0f) > 0.001f) {
                int harmTargetBin = (int)((i * _pitchFactor) + 0.5f); // Fast float-to-int cast
                
                if (harmTargetBin < SAMPLES / 2) {
                    // Mix the harmony voice in at 100% volume
                    newMag[harmTargetBin] += _frozen ? frozenMag[i] : liveMag; 
                    float harmShiftedFreq = (binFreq + (deltaPhase * INV_HOP)) * _pitchFactor; // Fast multiply
